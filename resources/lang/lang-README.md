## エラー表示を日本語化する方法
1. resources/lang以下にenフォルダがあり、それと同じ階層に[ja]フォルダを作成。
2. [言語ファイル](https://readouble.com/laravel/8.x/ja/auth-php.html)以下の4つの言語ファイルをresources/lang/jaフォルダ配下に設置。(Larvael9.xの場合はlang/jaフォルダ)
4つのファイルとは
- auth.php
```
<?php
...
```
のこと。
- validation.phpの部分を調整することによって「passwordが間違っています」のあたりの記述も変更でき、lang以下にja.jsonファイルを設置することにより、
```
{
  "Whoops! Something went wrong.":"サーバーで何か問題が発生しました"
}
```
のようにすることで変更できる